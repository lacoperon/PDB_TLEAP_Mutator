...
# Initial Run (1 ns)
n78.mpich3.wrapper pmemd.cuda.MPI -O -i dyn.in -p ../../TLEAP/{0}_gc_wat.prmtop -c ../../HEAT/{1}_gc_heat.rst -r {1}_gc_heatrestrained.rst -ref ../../HEAT/{1}_gc_heat.rst -o 5JUP_GC_heatrestrained.out

/share/apps/CENTOS6/amber/amber16/bin/ambpdb -p ../../TLEAP/{0}_gc_wat.prmtop -c {1}_gc_heatrestrained.rst > {1}_gc_heatrestrained.pdb

echo "neutral dynamics heatrestrained_{1}_GC is complete. View {1}_GC_heatrestrained.pdb with PyMol." | mailx -s "neutral_{1}_GC_heatrst" ejwilliams@wesleyan.edu
""".format(struct_name, struct_name.upper())

    f = open("NEUTRAL/HEAT_RESTRAINED/run_dyn.sh", "w")
    f.write(script_text)
    f.close()
...
"""
    ...
    f = open("NEUTRAL/HEAT_RESTRAINED/dyn.in", 'w')
    f.write(input_text)
    f.close()

if __name__ == "__main__":

    if not os.path.exists("NEUTRAL/HEAT_RESTRAINED/"):
        print("Making HEAT_RESTRAINED directory")
        os.makedirs("NEUTRAL/HEAT_RESTRAINED/")
